...
<div class="content-wrapper">
    ...
    <section class="content">
        <form action="{{route("creat_product")}}" method="post" enctype="multipart/form-data">
            @csrf 
                        ...
                        <div class="col-md-9">
                            ...
                            <div class="form-group">
                                ...
                                @error('smdetail')
                                    <span style="color: red; margin-left: 10px">{{$message}}</span>
                                @enderror
                                <textarea id="smdetail" name="smdetail" class="form-control summernote" rows="3">{{old("name")}}</textarea>
                            </div>
                            <div class="form-group">
                                ...
                                @error('detail')
                                    <span style="color: red; margin-left: 10px">{{$message}}</span>
                                @enderror
                                <textarea id="detail" name="detail" class="form-control summernote" rows="3">{{old("name")}}</textarea>
                            </div>
                        </div>
                        <div class="col-md-3">
                            <div class="form-group">
                                ...
                                <select id="category_id" name="category_id" class="form-control custom-select">    
                                    @foreach ($categori_list as $key => $item)
                                        @if ($key == 0)
                                        <option value="{{ $item->id}}">[--- Chọn loại sản phẩm ---]</option>
                                        @endif
                            ...
                            <div class="form-group">
                                ...
                                <select id="brand_id" name="brand_id" class="form-control custom-select">
                                    <option value="{{$brand_list[0]->id}}">[--- Chọn thương hiệu ---]</option>
