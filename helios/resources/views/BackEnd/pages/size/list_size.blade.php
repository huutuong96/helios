                        ...
                        <div class="card-header">
                            <div class="d-flex justify-content-between">
                                ...
                                <div class="card-tools">
                                    <a class="btn btn-primary" href="{{route("creat_size")}}">
                                        <i class="fa fa-plus"></i> Thêm mới
                                    </a>
                                    <a class="btn btn-secondary" href="{{route("size_list",["act"=>"thùng rác"])}}">
                                ...
                                <tbody>
                                    @foreach ($list_size as $row) 
                                        <tr>
                                            ...
                                            <td class="text-center">
                                                <b>Chên lệch giá:</b> {{$row->change}} %
                                            </td>
                                            <td class="text-center">
                                                @if ($row['status'] == 1) 
                                                    <a class="btn btn-sm btn-success" href="{{route("change_stutus_size",["id" => $row->id])}}"><i class="fa fa-toggle-on"></i></a>
                                                @else 
                                                    <a class="btn btn-sm btn-danger" href="{{route("change_stutus_size",["id" => $row->id])}}"><i class="fa fa-toggle-off"></i></a>
                                                @endif 
                                                <a class="btn btn-sm btn-info" href="{{route("edit_size",["id" => $row->id])}}"><i class="fa fa-edit"></i></a>
                                                <a class="btn btn-sm btn-danger" href="{{route("change_stutus_size",["id" => $row->id, "status" => 0])}}"><i class="fa fa-trash"></i></a>
                                            </td>
                                        </tr>
                                    @endforeach 
                                </tbody>
